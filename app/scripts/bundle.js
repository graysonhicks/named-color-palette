...
function setColorValues(color) {
  ...
  name.val(ntc.name("#" + hexValue)[1]);
  name.css("color", fontColorBasedOnBackground);

  var scheme = new ColorScheme();
  scheme.from_hue(hueValue).scheme(schemeMode.val()).variation(variationMode.val()); // Use the 'soft' color variation
...
function buildColorListBar(colors) {
  ...
  for (var i = 0; i < colors.length; i++) {
    ...
    var blockSpan = $("<div/>").addClass("named-color-block").css("background-color", "#" + colors[i]);
    ...
    var nameDiv = $("<div/>").html(ntcColor[1]).css("color", fontColorBasedOnBackground);
    blockSpan.attr("data-code", colors[i]);
    blockSpan.attr("data-name", ntcColor[1]);
    var hexDiv = $("<div/>").addClass("hex-span").css("color", fontColorBasedOnBackground).html("#" + colors[i]);
    ...
    blockSpan.append(nameDiv);
    blockSpan.append(hexDiv);
    col.append(blockSpan)
...
function setCurrentColor(color) {
  ...
  name.css("background-color", "#" + currentColor.toHex());
  spectrumCursor.css("background-color", "#" + currentColor.toHex());
}
...
schemeMode.change(function() {
  var color = tinycolor(currentColor);
  colorToPos(color);
});

variationMode.change(function() {
  var color = tinycolor(currentColor);
  colorToPos(color);
});
